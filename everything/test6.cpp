...
static void insertionBench(benchmark::State &s)
{
    ...
    if (capacity == pow(2, 24) && (loadfactor == 0.25 || loadfactor == 0.35))
    {
        totinsertkeys.reserve(insertkeys.size() + insertkeys.size() + insertkeys.size() + extrakeys.size());
    ...
    else if (capacity == pow(2, 24))
    {
        totinsertkeys.reserve(insertkeys.size() + insertkeys.size() + extrakeys.size());
        totinsertkeys.insert(totinsertkeys.end(), insertkeys.begin(), insertkeys.end());
        totinsertkeys.insert(totinsertkeys.end(), insertkeys.begin(), insertkeys.end());
        totinsertkeys.insert(totinsertkeys.end(), extrakeys.begin(), extrakeys.end());
    }
   ...
   // stab = succinsert;
    tableloadfactor = (double)table.getSize() / capacity;
    //s.counters["Teststab"] = stab;
    s.counters["Test"] =  tableloadfactor;
    s.counters["Totinsert"] = index;
    s.counters["Loadfactor"] = loadfactor;
}
/*
BENCHMARK(insertionBench<Linearprobing<uint64_t, uint64_t, MultiplyAddShift, BulkAllocator<uint64_t, uint64_t>>>)->Name("LPMult")->DenseRange(0, 2)->Unit(benchmark::kSecond);
